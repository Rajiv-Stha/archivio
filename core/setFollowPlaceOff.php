...
// variables; if they're present, continue attempting to save
  ...
  $type = $_POST['t'];
      ...
      $sql2 = "DELETE from `followers` WHERE  (follow_user_id = '$to_user' AND follower_user_id = '$user_id' AND follower_type = 'place')";
      //echo $sql2;

    $result2 = $db->query($sql2); 

$icon = " <img src=\"$path_url/assets/img/footsteps.svg\"  fill = \"#fff\" class=\"icon_footsteps\">";
                  $follow_button = "<div class=\"btn btn-gr bg-place shadowed border-0 btn-block btn-follow\" onclick=\"setFollowPlaceOn($to_user, $type);\">
                      $icon
